...
CREATE PROCEDURE `bhmc`.`MembershipReport`(IN currentEventId INT, IN previousEventId INT)
BEGIN
	SELECT 
		 rs.registration_id 
		,rp.id AS player_id
...
CREATE PROCEDURE `bhmc`.`SearchPlayers`(IN pattern VARCHAR(20), IN playerId INT, IN eventId INT, IN seasonEventId INT, IN previousSeasonEventId INT)
BEGIN
	SELECT
		 p.id 
		,p.first_name 
...
BEGIN
	...
	FROM `bhmc`.damcup_seasonlongpoints slp 
	JOIN `bhmc`.register_player p ON slp.player_id = p.id 
	JOIN `bhmc`.events_event e ON slp.event_id = e.id
	WHERE e.season = season
	GROUP BY 
		 p.first_name
		,p.last_name 
