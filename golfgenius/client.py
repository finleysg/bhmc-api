...
class GolfGeniusAPIClient:
    ...
    def __init__(self, api_key: Optional[str] = None, base_url: str = "https://www.golfgenius.com"):
        ...
        logger.info("Golf Genius API client initialized",
    ...
    def _make_request(self, method: str, endpoint: str, params: Optional[Dict] = None,
        ...
        for attempt in range(self.max_retries + 1):
            try:
                logger.debug("Making Golf Genius API request",
                            method=method, url=url, params=params, attempt=attempt + 1)
                ...
                logger.debug("Golf Genius API response",
                    ...
                    if attempt < self.max_retries:
                        ...
                        if retry_after:
                            try:
                                ...
                                retry_delay = min(int(retry_after), self.retry_delay_max)
                        ...
                        logger.error("Rate limit exceeded, max retries reached",
                                   attempts=attempt + 1)
                        raise GolfGeniusRateLimitError("Rate limit exceeded after maximum retries")
                
                # Handle authentication errors (don't retry)
                if response.status_code == 401:
                    raise GolfGeniusAuthError("Invalid API key or authentication failed")
        ...
        if last_exception:
            if isinstance(last_exception, requests.exceptions.Timeout):
                ...
                raise GolfGeniusAPIError(f"Request failed after retries: {str(last_exception)}")
        
        # Fallback error
        raise GolfGeniusAPIError("Request failed after maximum retries")
    
    def get_master_roster(self, page: Optional[int] = None, photo: bool = False) -> List[Dict[str, Any]]:
        """
        ...
        logger.info("Retrieved master roster", 
    ...
    def get_categories(self) -> List[Dict[str, Any]]:
        """
        Get all categories from Golf Genius
        
        Returns:
            List of category dictionaries
        """
        endpoint = f"/api_v2/{self.api_key}/categories"
        response = self._make_request("GET", endpoint)
    ...
    def get_events(self, page: Optional[int] = None, season_id: Optional[str] = None,
                   category_id: Optional[str] = None, directory_id: Optional[str] = None,
