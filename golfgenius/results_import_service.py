...
from events.models import Event, Tournament, TournamentResult
...
class ResultsImportResult:
    ...
    def add_error(self, error: str):
        ...
        logger.error("Results import error", tournament_id=self.tournament_id, error=error)
...
class ResultsImportService:
    ...
    def import_stroke_play_results(self, event_id: int) -> List[ResultsImportResult]:
        ...
        if not tournaments.exists():
            logger.warning("No stroke play tournaments found for event", event_id=event_id)
            return results
    ...
    def _import_tournament_results(self, tournament: Tournament) -> ResultsImportResult:
        ...
        try:
            ...
            if not tournament.event.gg_id:
                result.add_error("Event must first be synced with Golf Genius. Run Event Sync process.")
                return result

            # Check if round has Golf Genius ID
            if not tournament.round.gg_id:
                result.add_error("Round must first be synced with Golf Genius. Run Event Sync process.")
                return result

            # Check if tournament has Golf Genius ID
            if not tournament.gg_id:
                result.add_error("Tournament must first be synced with Golf Genius. Run Event Sync process.")
                return result

            # Delete existing results for this tournament (idempotent operation)
            with transaction.atomic():
                deleted_count = TournamentResult.objects.filter(tournament=tournament).delete()[0]
                logger.info(
            ...
            logger.exception("Unexpected error in tournament results import", tournament_id=tournament.id)

        return result

    def _process_tournament_results(
        self, tournament: Tournament, gg_results: Dict[str, Any], result: ResultsImportResult
    ):
        """
        ...
        for scope in event_data["scopes"]:
            ...
            for aggregate in scope["aggregates"]:
                try:
                    self._process_player_result(tournament, aggregate, flight_name, result)
                except Exception as e:
                    result.add_error(f"Error processing player result: {str(e)}")
                    logger.exception("Error processing player result", tournament_id=tournament.id)

    def _process_player_result(
        self, tournament: Tournament, aggregate: Dict[str, Any], flight: str, result: ResultsImportResult
    ):
        """
        ...
        if not member_cards:
            result.add_error(f"No member cards found for aggregate {aggregate.get('name', 'Unknown')}")
            return

        member_card_id = str(member_cards[0].get("member_card_id", ""))
        if not member_card_id:
            result.add_error(f"No member card ID found for {aggregate.get('name', 'Unknown')}")
            return
        ...
        try:
            ...
            result.add_error(f"Player not found with Golf Genius ID {member_card_id} for {aggregate.get('name', 'Unknown')}")
            return
        except Player.MultipleObjectsReturned:
            result.add_error(f"Multiple players found with Golf Genius ID {member_card_id}")
            return
