...
from register.models import Player, RegistrationSlot, RegistrationFee
...
class PlayerSyncService:
    ...
    def get_sync_status(self) -> Dict:
        ...
        return {
            ...
            "sync_percentage": round((synced_players / total_events * 100) if total_events > 0 else 0, 2)
        }
...
class RosterExportResult:
    ...
    def add_success(self, player_name: str, gg_member_id: str, api_response: Dict, operation: str = "created"):
        ...
        logger.info("Player exported successfully",
                    player=player_name,
                    gg_member_id=gg_member_id,
                    operation=operation)
...
class RosterService:
    ...
    def export_roster(self, event_id: int, round_id: Optional[str] = None) -> RosterExportResult:
        ...
        try:
            ...
            try:
                ...
                logger.info("Found BHMC event for roster export",
                           event_id=event_id,
            ...
            ).select_related('player', 'registration', 'event').prefetch_related('fees__event_fee')
            ...
            logger.info("Found players to export",
                       event_name=event.name,
                       total_players=result.total_players)
            ...
            try:
                ...
                logger.info("Built roster external_id -> member_id map",
                           event_name=event.name, mapped_count=len(roster_map))
            except Exception as e:
                logger.warning("Failed to fetch event roster from Golf Genius, proceeding with creates",
                               event_name=event.name, error=str(e))
                roster_map = {}
            ...
            logger.error("Roster export operation failed", error=str(e), event_id=event_id)
    ...
    def export_single_player(self, event_id: int, player_id: int, round_id: Optional[str] = None) -> RosterExportResult:
        ...
        try:
            ...
            try:
                event = Event.objects.get(id=event_id)
                logger.info("Found BHMC event for single player export",
                           event_id=event_id,
                           event_name=event.name,
                           start_date=str(event.start_date))
            except Event.DoesNotExist:
            ...
            ).select_related('player', 'registration', 'event').prefetch_related('fees__event_fee').first()

            if not slot:
                result.add_error("SYSTEM", f"No registration slot found for player {player_id} in event {event_id}")
                return result
            ...
            try:
                ...
                for member in gg_members:
                    ...
                    if ext_id and member_id:
                        roster_map[str(ext_id)] = str(member_id)
                logger.info("Built roster external_id -> member_id map for single export",
                            event_id=event_id, mapped_count=len(roster_map))
            except Exception as e:
                logger.warning("Failed to fetch event roster from Golf Genius for single export, proceeding with create",
            ...
            logger.error("Single player export operation failed", error=str(e), event_id=event_id, player_id=player_id)
    ...
    def _export_player(self, slot: RegistrationSlot, round_id: str, result: RosterExportResult, roster_map: Optional[Dict[str, str]] = None):
        ...
        try:
            ...
            if roster_map and roster_map.get(external_id):
                ...
                api_response = self.api_client.update_member_registration(slot.event.gg_id, gg_member_id, member_data)
                result.add_success(player_name, str(gg_member_id), api_response, operation="updated")
            else:
                # Create new member registration
                api_response = self.api_client.create_member_registration(slot.event.gg_id, member_data)
                gg_member_id = api_response.get('member_id') or api_response.get('id')
                result.add_success(player_name, str(gg_member_id), api_response, operation="created")
    ...
    def _get_skins_amounts(self, slot: RegistrationSlot) -> tuple[int, int]:
        ...
        try:
            ...
            logger.warning("Failed to get skins amounts",
                          slot_id=slot.id,
                          error=str(e))
